...
const OutdoorProducts = ({ pageContext, location, ...restProps }) => {
    const {sectionStyle} = restProps;
    const allProducts = ProdQuery();

    return (
        <Layout location={location}>
            <SEO title="Firepits, Firetables" />
            <Header/>
            <PageHeader 
                pageContext={pageContext} 
                location={location}
                title="Firepits, Firetables"
            />
...
OutdoorProducts.propTypes = {
    sectionStyle: PropTypes.object
}

OutdoorProducts.defaultProps = {
    sectionStyle: {
        pt: '15px',
        pb: '100px',
        responsive: {
