...
import {SectionWrap, CaseInfo, CaseTitle, CaseCat, CaseText, CaseImage} from './case-sudy-area.style'

const CaseStudyArea = (props) => {
    const caseData = useStaticQuery(graphql `
        query ServiceCaseStudyQuery {
            servicesJson(id: {eq: "services-case-contnet"}) {
                title
                subtitle
              }
    ...
    `)
    const {title, subtitle} = caseData.servicesJson;
                        ...
                        <div className="item" key={caseStudy.node.id}>
                            <Row>
                                <Col lg={{span: 6, order: 1}} xs={{span: 12, order: 2}}>
                                    <CaseInfo>
                                        {caseStudy.node.title && <CaseTitle><Link to={`/case-sudy/${caseStudy.node.fields.slug}`}>{caseStudy.node.title}</Link></CaseTitle>}
